...
#This R pipeline was written by Katherine Dowdell and Hannah Greenwald Healy for the associated manuscript, Dowdell and Healy et al. 2023 (DOI: 10.1039/d3ew00278k), Legionella pneumophila occurence in reduced-occupancy buildings in 11 cities during the COVID-19 pandemic

#If code is re-run, conflicts between dplyr and ggbiplot, which uses plyr.
...
raw_data <- read_excel("Lp_study_data_2023.xlsx", sheet = "sample_data")
bldg_data<- read_excel("Lp_study_data_2023.xlsx", sheet = "building_metadata")
qPCR_results<- read_excel("Lp_study_data_2023.xlsx", sheet = "qPCR_results")
ddpcr_raw<- read_excel("Lp_study_data_2023.xlsx", sheet = "ddPCR_results")
raw_comb_meter<- read_xlsx("Lp_study_data_2023.xlsx", sheet = "water_use")
```
...
##wilcoxon by disinfectant- legiolert result
wilcox.test(free$legiolert_mpn100ml,mono$legiolert_mpn100ml)
#p-value = 5.326e-08

##wilcoxon by disinfectant- qPCR/ddPCR result
wilcox.test(free$gc_L,mono$gc_L)
#p-value =0.973

##wilcoxon by disinfectant- qPCR/ddPCR result- positive only
free_qpcr_pos<- free %>%
  filter(BLoD==2)
mono_qpcr_pos<- mono %>%
  filter(BLoD==2)
...
##wilcoxon by condition (fd v. flushed)
wilcox.test(fd$legiolert_mpn100ml,fl$legiolert_mpn100ml)
#p-value = 0.9069
```
...
fig_1a<- leg_fig1 %>%
  ...
  geom_jitter(aes(fill= condition, shape=condition), size=2, alpha=0.7,  position=position_dodge(0.8)) + 
...
fig_1a
tiff("fig_1a_highres.tiff", units = "in", width=7, height=5, res=800)
print(fig_1a)
dev.off()
```
...
fig_1b<- df %>%
  ...
  new_scale_fill()+
  geom_jitter(aes(fill=condition, shape=condition), size=2, alpha=0.7,  position=position_dodge(0.3), show_guide = FALSE)+
  scale_shape_manual(values = c(22, 23))+
  scale_fill_viridis_d(begin = 0.3, end = 0.8, label = c("First Draw", "Flushed"))+
  geom_hline(yintercept = 10, linetype="dashed")+
  geom_hline(yintercept = 22726, linetype="dotted")+
...
fig_1b
tiff("fig_1b_highres.tiff", units = "in", width=3, height=4, res=800)
print(fig_1b)
dev.off()
```

```{r}
##water use data and Fig S2
comb_water_use<- raw_comb_meter %>%
  mutate(year=as.character(year)) %>%
  mutate(mon_year=as.Date(mon_year)) %>%
  mutate(mon=lubridate::month(mon_year, abbr=T, label=T)) %>%
  mutate(as_L=total_gal*3.7854)

coeff<- 3.7854
figure_s2<- comb_water_use %>%
  ggplot(aes(x=mon,group=year, color=year))+
  geom_point(aes(y=total_gal))+
  geom_line(aes(y=total_gal))+
  scale_y_log10(sec.axis=sec_axis(~.*3.7854, name="Liters"))+
  scale_x_discrete(breaks=c("Jan", "Mar", "May", "Jul", "Sep", "Nov"))+
  scale_color_viridis_d(option = "D", end=0.9)+
  labs(x="Month", y="Water Use (gallons)", color="Year")+
  ...
  facet_wrap(~building_id)
figure_s2

tiff("figure_s2.tiff", units = "in", width=8, height=6, res=600)
print(figure_s2)
dev.off()
```

```{r}
##Figure S4 - L. pneumophila gene copies by building
fig_s4 <- df %>%
...
fig_s4
tiff("fig_s4.tiff", units = "in", width=8, height=4, res=600)
print(fig_s4)
dev.off()
```

```{r}
##Figure S5 - L. pneumophila gc/L by disinfectant and sample type
...
fig_s5<- df %>%
  ...
  theme(legend.position="right",
        ...
        legend.text =element_text(size=8,face="bold", color="black"))
fig_s5
tiff("fig_s5.tiff", units = "in", width=7, height=5, res=400)
print(fig_s5)
dev.off()
```
...
## number of samples w/ measurable Cl
cl_dl_1<- 0.02
cl_dl_2<- 0.05

cl_low<- with_cl %>%
  mutate(cl_tot_dl= ifelse(site_id %in% c("AZ", "CA", "IN",  "MA", "QC", "WV"), 0.02,0.05)) %>%
  mutate(cl_detect= ifelse(cl_tot_mgl>=cl_tot_dl, "detect", "BDL"))
...
```{r}
##Figure S6 - Cl, pH, and temp box plots by site
FigS6_data<- df_long %>%
...
fig_s6
tiff("fig_s6.tiff", units = "in", width=8, height=7, res=800)
print(fig_s6)
dev.off()
```
...
fig_2a
tiff("fig_2a.tiff", units = "in", width=2.5, height=3, res=600)
print(fig_2a)
dev.off()
...
fig_2b<- with_ph %>%
  ...
  theme(
    ...
    plot.margin=unit(c(0,0,0,0), "cm"))
fig_2b
tiff("fig_2b.tiff", units = "in", width=2.5, height=3, res=600)
print(fig_2b)
dev.off()
...
fig_2c<- with_temp %>%
  ...
  theme(
    ...
    plot.margin=unit(c(0,0,0,0), "cm"))
fig_2c
tiff("fig_2c.tiff", units = "in", width=2.5, height=3, res=600)
print(fig_2c)
dev.off()
...
tiff("fig_2_highres.tiff", units = "in", width=6, height=4, res=800)
print(fig_2)
dev.off()
```
...
fig_3a
tiff("fig_3a.tiff", units = "in", width=6, height=4, res=300)
print(fig_3a)
dev.off()
```
...
fig_3b
tiff("fig_3b.tiff", units = "in", width=6, height=4, res=300)
print(fig_3b)
dev.off()
```
...
fig_3
tiff("fig_3.tiff", units = "in", width=9, height=4, res=300)
print(fig_3)
dev.off()
```
...
cor.test(qPCR_ddpcr$gc_L, qPCR_ddpcr$legiolert_mpn100ml, method=c("kendall")) 
...
fig_s7d<- legio_and_cl_qpcr_binned %>%
...
```{r}
fig_s7<- ggarrange(fig_s7a, fig_s7b, fig_s7c,fig_s7d, ncol=2, nrow=2, align = "hv",hjust=-0.05,  labels= c("A", "B", "C", "D"), font.label = list(size = 9, color = "black", face = "bold", family = NULL))
fig_s7
tiff("fig_s7.tiff", units = "in", width=10, height=8, res=600)
print(fig_s7)
dev.off()
```

```{r}
## Figure S8 - L. pneumophila gc/L v. Chlorine 
fig_s8<- with_cl %>%
  mutate(disinfectant=factor(disinfectant, order = TRUE, levels =c("free_chlorine", "chloramines", "none"))) %>%
  mutate(BLoD=factor(BLoD, order = TRUE, levels =c("0", "1", "2"))) %>%
  filter(!is.na(BLoD)) %>%
...
fig_s8
tiff("fig_s8.tiff", units = "in", width=6, height=4, res=400)
print(fig_s8)
dev.off()
```

```{r}
## Figure S10 - L. pneumophila gc/L v. temp
fig_s10<- with_temp %>%
  mutate(disinfectant=factor(disinfectant, order = TRUE, levels =c("free_chlorine", "chloramines", "none"))) %>%
  mutate(BLoD=factor(BLoD, order = TRUE, levels =c("0", "1", "2"))) %>%
  filter(!is.na(BLoD)) %>%
...
fig_s10
tiff("fig_s10.tiff", units = "in", width=6, height=4, res=400)
print(fig_s10)
dev.off()
```

```{r}
## Figure S11 - L. pneumophila gc/L v. pH
fig_s11<- with_ph %>%
  mutate(disinfectant=factor(disinfectant, order = TRUE, levels =c("free_chlorine", "chloramines", "none"))) %>%
  mutate(BLoD=factor(BLoD, order = TRUE, levels =c("0", "1", "2"))) %>%
  filter(!is.na(BLoD)) %>%
...
fig_s11
tiff("fig_s11.tiff", units = "in", width=6, height=4, res=400)
print(fig_s11)
dev.off()
```

```{r}
## Figure S12 - L. pneumophila MPN/L v. dissolved oxygen
fig_s12<- with_DO %>%
...
fig_s12
tiff("fig_s12.tiff", units = "in", width=6, height=4, res=400)
print(fig_s12)
dev.off()
```

```{r}
## Figure S13 - L. pneumophila MPNL v. conductivity
fig_s13<- with_cond %>%
...
fig_s13
tiff("fig_s13.tiff", units = "in", width=6, height=4, res=300)
print(fig_s13)
dev.off()
```
...
```{r}
## Figure S9 - Temperature in FD and Flushed PAIRED Samples by Tap Temp
...
fig_s9
tiff("fig_s9.tiff", units = "in", width=6, height=4, res=300)
print(fig_s9)
dev.off()
``` 

```{r}
## Figure S14 - Chlorine in FD and Flushed PAIRED Samples by Tap Temp
...
fig_s14
tiff("fig_s14.tiff", units = "in", width=6, height=4, res=300)
print(fig_s14)
dev.off()
``` 

```{r}
## Figure S15 - L. pneumophila FD v. Flushed for PAIRED samples by building
fig_s15<- df_paired %>%
...
fig_s15
tiff("fig_s15.tiff", units = "in", width=6, height=4, res=300)
print(fig_s15)
dev.off()
```

```{r}
## Figure S16 - L. pneumophila gene copies per liter for FD and Flushed PAIRED samples by building
...
fig_s16
tiff("fig_s16.tiff", units = "in", width=6, height=4, res=300)
print(fig_s16)
dev.off()
``` 
...
```{r}
###### NOTE! Once ggbiplot is loaded there can be conflicts with dplyr! Skip this section or re-start R to run the code in the above sections
...
keep= c( "site_id", "pH", "temp_c", "cl_tot_mgl", "legiolert_mpn100ml" ,  "disinfectant",  "tot_floors", "building_age", "building_id")
...
summary(df.pca)
fig_s18b <- ggbiplot(df.pca, choices= c(1,2), ellipse=F, groups= df_quant$site_id, labels.size = 6, varname.size = 3.5)+
...
fig_s18b
tiff("fig_s18b.tiff", units = "in", width=5, height=5, res=300)
print(fig_s18b)
dev.off()
...
df.pca <- prcomp(df_quant[keep.pca], center= TRUE, scale.=TRUE)
summary(df.pca)
fig_s18a<- ggbiplot(df.pca, choices= c(1,2), ellipse=TRUE, groups= df_quant$pos_neg, labels.size = 6, varname.size = 4)+
...
fig_s18a
tiff("fig_s18a.tiff", units = "in", width=5, height=5, res=300)
print(fig_s18a)
dev.off()
```
...
fig_s18<- ggarrange(fig_s18a,fig_s18b, ncol = 2,nrow=1, common.legend = FALSE, legend = "bottom", align = "hv",hjust=-1,  labels= c("A", "B"), font.label = list(size = 9, color = "black", face = "bold", family = NULL))
fig_s18
tiff("fig_s18.tiff", units = "in", width=8, height=5, res=400)
print(fig_s18)
dev.off()
```
...
```{r}
## df with only quantitative variables for modeling
model_data<- df_famd %>%
...
```{r}
##summary stats by water source
groundwater<- df_famd %>%
  filter(water_source=="ground")
median(groundwater$legiolert_mpn100ml) #0.5

surface<- df_famd %>%
  filter(water_source=="surface")
median(surface$legiolert_mpn100ml) #0.5

surface_ground<- df_famd %>%
  filter(water_source=="surface_ground")
median(surface_ground$legiolert_mpn100ml) #2.3

surface_cl<- surface %>%
  filter(disinfectant=="free_chlorine")
median(surface_cl$legiolert_mpn100ml) #0.5

surface_ground_cl<- surface_ground %>%
  filter(disinfectant=="free_chlorine")
  median(surface_ground_cl$legiolert_mpn100ml) #5.8
  
##positivity
ground_lppos<- groundwater %>%
  filter(legiolert_mpn100ml>0.5) #3 samples
  
surface_lppos<- surface %>%
  filter(legiolert_mpn100ml>0.5) #20 samples

surface_ground_lppos<- surface_ground %>%
  filter(legiolert_mpn100ml>0.5) #65 samples
```

```{r}
##wilcoxon by water source
wilcox.test(surface$legiolert_mpn100ml,surface_ground$legiolert_mpn100ml)
wilcox.test(surface$legiolert_mpn100ml,groundwater$legiolert_mpn100ml)
wilcox.test(surface_ground$legiolert_mpn100ml,groundwater$legiolert_mpn100ml)
```

```{r}
##Figure S17 - L. pneumophila MPN/L by disinfectant, sample type, and source water type
fig_s17<- df_famd %>%
  filter(disinfectant !="none")%>%
  mutate(disinfectant=factor(disinfectant, order = TRUE, levels =c("free_chlorine", "chloramines"))) %>%
  mutate(water_source=factor(water_source, order = TRUE, levels =c("ground", "surface",   "surface_ground"))) %>%
  ggplot(aes(x=water_source, y= legiolert_mpn100ml*10))+
  geom_boxplot(aes(fill=disinfectant), outlier.shape = NA, alpha=0.3)+
  geom_jitter(aes(fill=disinfectant, shape=condition), size=2, alpha=0.7,position=position_jitterdodge(jitter.width = 0.16,jitter.height = 0, dodge.width = 0.9))+
  scale_shape_manual(values = c(22, 23))+
  scale_fill_manual(values=c("#33638d", "#95d840"), labels=c("Free Chlorine","Monochloramine"))+
  scale_color_viridis_d(begin = 0.3, end = 0.8, label = c("First Draw", "Flushed"))+
  geom_hline(yintercept = 10, linetype="dashed")+
  geom_hline(yintercept = 22726, linetype="dotted")+
  scale_x_discrete(labels=c("Groundwater","Surface Water","Surface & Groundwater"))+
  labs(x= "Water Source", y= "L. pneumophila (MPN/L)")+
  scale_y_log10(breaks = c(1, 10, 100, 1E3,1E4, 1E5), limits = c(1, 1E5), labels = trans_format("log10", math_format(10^.x)), expand=c(0,0))+
  theme_classic(base_size=10)+
  theme(legend.position="top",
        axis.title.y = element_text(size=10,face="bold", color="black"), 
        ...
        legend.text =element_text(size=8,face="bold", color="black"))
fig_s17
tiff("fig_s17.tiff", units = "in", width=7, height=5, res=400)
print(fig_s17)
dev.off()
```
