...
```{r} 
##read in data files-> save to same directory as R code
...
fig_3b<- gc_mpn_quanti %>% 
  ...
  #scale_y_log10(breaks = c(1, 1E1, 1E2, 1E3,1E4, 1E5, 1E6), limits = c(1, 1E6), expand=c(0,0))+
  #scale_x_log10(breaks = c(1, 10, 100, 1E3,1E4, 1E5, 1E6, 1E7, 1E8), limits = c(1, 1E8), expand=c(0,0))+
...
#cor.test(df$gc_L, df$legiolert_mpn100ml, method=c("pearson")) #not significantly correlated
#cor.test(df$gc_L, df$legiolert_mpn100ml, method=c( "spearman")) #can't handle ties
...
geom_boxplot(aes(fill=condition),outlier.shape = NA)+
  ...
  scale_fill_manual(values=as.vector(polychrome(29)), guide="none")+
  #guides(fill=guide_legend(override.aes=list(shape=21)))+
  #geom_line(aes(group=sample_loc), size=0.25, linetype="dotted")+
  scale_x_discrete(labels=c("First-Draw","Flushed"))+
  theme_classic(base_size=12)+
  theme(legend.position = "right",
        legend.title = element_blank(),
